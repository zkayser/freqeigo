module Japanese
  ...
  # 4/2/2016 -> This module essentially functions properly, though it is not tested and it can be refactored
  # to make it much shorter and easier to follow. Also, as it stands right now, the programmer has to choose what part of
  # speech a given verb is manually. To make it more convenient to use, it should be updated to infer what part of speech
  # the verb is algorithmically.
  
  # Be sure to go over this list for a complete list of exceptions for 
  # consonant verbs that end in -eru -iru: https://en.wikipedia.org/wiki/Japanese_consonant_and_vowel_verbs#List_of_consonant_stem_verbs_ending_in_iru
  ...
  def set_verb_stem_form
    ...
    # word.save! => Comment this out now for experimental purposes 
  end
  ...
  def set_prohibitive_form
    unless self.part_of_speech == "v-aru" || self.part_of_speech == "v5r-i"
      self.conjugations[:prohibitive] = self.word + "な"
      self.hiragana_forms[:prohibitive] = self.hiragana + "な"
      self.romaji_forms[:prohibitive] = self.reading + " na"
    else
  ...
  def set_adjective_conjugations
    ...
    self.conjugations[:adjective_conjugations] = { present: self.word,
                                                   present_polite: self.word + "です",
                                                   past: stem + "かった",
                                                   past_polite: self.word + "でした",
                                                   te_form: stem + "くて"
                                                  }
    ...
    self.romaji_forms[:adjective_conjugations] = { present: self.reading,
                                                   present_polite: self.reading + " desu",
                                                   past: romaji_stem + "katta",
                                                   past_polite: self.reading + " deshita",

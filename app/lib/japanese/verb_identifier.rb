    ...
    def resolve_ru_verb_class
      ...
      unless irregular?
        ...
        RU_IRREGULAR_MAPPING.each do |k, v|
          self.part_of_speech = v if self.word == k
        end
      end
    end
    ...
    def resolve_verb_class
      ...
      if part_of_speech == "verb"
        case self.word[-1]
          ...
          when "く"
            self.part_of_speech = "v5k" unless self.word == "行く"
          when "む"
          ...
          when "う"
            self.part_of_speech = "v5u" unless self.word == "問う"
        end
      end
    end
      ...
      def ends_in_ru?
        unless self.word.blank?
          self.word[-1] == "る" ? true : false
        else 
      ...
      def ambiguous?
        self.word == self.hiragana && self.word.in?(AMBIGUITY_IF_HIRAGANA_ONLY) ? true : false
      end
      ...
      def is_consonant_verb?
        if self.ends_in_iru_eru? && !ambiguous?
          self.word.in?(CONSONANT_VERBS) ? true : false
        end
      end
